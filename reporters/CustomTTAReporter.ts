...
class CustomTTAReporter implements Reporter {
    ...
    onTestEnd(test: TestCase, result: TestResult): void {
        ...
        for (const attachment of result.attachments) {
            if (attachment.contentType === 'image/png') {
                ...
                try {
                    ...
                    if (attachment.name) stepScreenshots.set(attachment.name, `screenshots/${screenshotName}`);
                } catch { console.warn(`Failed to save screenshot: ${attachment.name}`); }
            }
            ...
            if (attachment.contentType === 'video/webm' && attachment.path) {
                ...
                } catch { console.warn(`Failed to copy video: ${attachment.path}`); }
            }
            ...
            if (attachment.name === 'trace' && attachment.path) {
                ...
                } catch { console.warn(`Failed to copy trace: ${attachment.path}`); }
            }
        }
        ...
        for (const step of currentTestSteps) {
            for (const [name, screenshotPath] of stepScreenshots) {
                ...
                if (nameLower.startsWith(stepIndexPattern)) { step.screenshot = screenshotPath; break; }
                const stepNumPattern1 = `step_${(step.stepIndex || 0) + 1}_`;
                const stepNumPattern2 = `step ${(step.stepIndex || 0) + 1}`;
                if (nameLower.includes(stepNumPattern1) || nameLower.includes(stepNumPattern2)) { step.screenshot = screenshotPath; break; }
                const cleanedName = nameLower.replace(/step[-_]?\d+[-_:]?/i, '').trim();
                if (cleanedName && (titleLower.includes(cleanedName) || cleanedName.includes(titleLower.substring(0, 20)))) { step.screenshot = screenshotPath; break; }
            }
        }
        ...
        while (parent) {
            if (parent.title) describePath.unshift(parent.title);
            parent = parent.parent;
        }

        const tagMatches: string[] = test.title.match(/@\w+/g) ?? [];
        ...
        const testData: TestData = {
            ...
            ...(result.error?.stack !== undefined && { errorStack: result.error.stack }),
            tags: tagMatches,
        };
        ...
        if (!this.fileGroups.has(fileName)) {
            this.fileGroups.set(fileName, { file: fileName, describes: new Map(), stats: { passed: 0, failed: 0, skipped: 0, total: 0 } });
        }
        ...
        if (!fileGroup.describes.has(describeKey)) fileGroup.describes.set(describeKey, []);
    ...
    private associateLogsWithSteps(_test: TestCase, result: TestResult, testSteps: StepData[]): void {
        ...
        for (const step of testSteps) { if (!step.consoleLogs) step.consoleLogs = []; }
        ...
        for (const chunk of (result.stdout || [])) {
            if (typeof chunk === 'string') allLogs.push(...chunk.split('\n').filter((l: string) => l.trim()));
            else if (Buffer.isBuffer(chunk)) allLogs.push(...chunk.toString().split('\n').filter((l: string) => l.trim()));
        }
        for (const chunk of (result.stderr || [])) {
            if (typeof chunk === 'string') allLogs.push(...chunk.split('\n').filter((l: string) => l.trim()).map((l: string) => `[stderr] ${l}`));
            else if (Buffer.isBuffer(chunk)) allLogs.push(...chunk.toString().split('\n').filter((l: string) => l.trim()).map((l: string) => `[stderr] ${l}`));
        }
        if (allLogs.length === 0) return;

        const stepTitlePatterns: RegExp[] = testSteps.map(step => new RegExp(step.title.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i'));
        ...
        const unassignedLogs = allLogs.filter((_: string, idx: number) => !assignedLogs[idx]);
            ...
            if (stepsNeedingLogs.length > 0) {
                ...
                for (let stepIdx = 0; stepIdx < testSteps.length && logIdx < unassignedLogs.length; stepIdx++) {
                    ...
                    if (step.consoleLogs!.length === 0) {
                        for (let i = 0; i < logsForThisStep; i++) step.consoleLogs!.push(unassignedLogs[logIdx++]!);
                    }
                }
            } else {
    ...
    private generateHistoryPage(reportDir: string): void {
        const files = fs.readdirSync(reportDir).filter((f: string) => f.startsWith('report_') && f.endsWith('.html')).sort().reverse();
...
<body>
    ...
    <div class="report-list">
        ${files.map((f: string, i: number) => {
            ...
            return `<div class="report-item"><a href="${f}" class="report-link">${f}${latestBadge}</a><span class="report-date">${dateStr}</span></div>`;
        }).join('\n')}
    </div>
</body>
</html>`;
...
<body>
    ...
    private generateMetaSection(browserName: string, platform: string): string {
        ...
        <div class="stats-dashboard">
            <div class="stat-card"><div class="stat-value">${this.suiteStats.total}</div><div class="stat-label">Total Tests</div></div>
            <div class="stat-card passed"><div class="stat-value">${this.suiteStats.passed}</div><div class="stat-label">Passed</div></div>
            <div class="stat-card failed"><div class="stat-value">${this.suiteStats.failed}</div><div class="stat-label">Failed</div></div>
            <div class="stat-card skipped"><div class="stat-value">${this.suiteStats.skipped}</div><div class="stat-label">Skipped</div></div>
            <div class="stat-card"><div class="stat-value">${passRate}%</div><div class="stat-label">Pass Rate</div></div>
            <div class="stat-card"><div class="stat-value">${this.formatDuration(totalDuration)}</div><div class="stat-label">Duration</div></div>
        </div>
        <div class="meta-section">
            <div class="meta-item"><span class="meta-label">Environment</span><span class="env-badge">🌐 ${env.toUpperCase()}</span></div>
            <div class="meta-item"><span class="meta-label">Browser</span><span class="browser-badge">🌍 ${browserName}</span></div>
            <div class="meta-item"><span class="meta-label">Platform</span><span class="meta-value">${platform}</span></div>
            <div class="meta-item"><span class="meta-label">Workers</span><span class="meta-value">${this.config.workers || 1}</span></div>
            <div class="meta-item"><span class="meta-label">Run ID</span><span class="meta-value" style="font-family:monospace;font-size:12px">${this.runId}</span></div>
            <div class="meta-item"><span class="meta-label">Started</span><span class="meta-value">${this.formatTime(this.startTime)}</span></div>
        </div>`;
    }
    ...
    private generateTestTable(): string {
        let html = `<div class="test-table-container"><table class="test-table">
            <thead><tr>
                <th>S.No</th><th>Suite</th><th>Test Name</th><th>Author</th><th>Priority</th>
                <th>Tags</th><th>File</th><th>Start Time</th><th>End Time</th>
        ...
        for (const test of this.testResults) {
            ...
            html += `
                <tr class="test-row ${statusClass}" data-test-id="${test.id}" data-tags="${tagsData}">
                    ...
                    <td class="col-testname"><span class="test-name-link" onclick="toggleTestDetail('${test.id}')">${this.escapeHtml(test.title)}</span></td>
                    ...
                    <td class="col-screenshot">${firstScreenshot ? `<a href="${firstScreenshot}" target="_blank" class="screenshot-link">📷 View</a>` : 'N/A'}</td>
                    <td class="col-video">${test.video ? `<a href="${test.video}" target="_blank" class="video-link-cell">▶️ Play</a>` : 'N/A'}</td>
                    <td class="col-trace">${test.trace ? `<a href="${test.trace}" target="_blank" class="trace-link-cell">📁 View</a>` : 'N/A'}</td>
                </tr>
                <tr class="test-detail-row" id="detail-row-${test.id}" style="display:none;">
                    <td colspan="14"><div class="test-detail" id="detail-${test.id}">${this.generateTestDetailPanel(test)}</div></td>
                </tr>`;
            serialNo++;
        }
    ...
    private generateTestDetailPanel(test: TestData): string {
        ...
        if (test.error) {
            html += `<div class="detail-section error-section">
                <div class="section-header" onclick="toggleSection(this)"><span class="section-arrow">▼</span> Errors</div>
                <div class="section-content"><div class="error-box">
        ...
        if (test.steps.length > 0) {
            html += `<div class="detail-section steps-section">
                <div class="section-header" onclick="toggleSection(this)"><span class="section-arrow">▼</span> Test Steps</div>
                <div class="section-content"><div class="steps-list">`;
            ...
            for (let stepIndex = 0; stepIndex < test.steps.length; stepIndex++) {
                ...
                html += `<div class="step-item-container">
                ...
                if (step.consoleLogs && step.consoleLogs.length > 0) {
                    html += `<div class="step-console"><div class="step-console-header">📋 Console Output (${step.consoleLogs.length} lines)</div>
                        <div class="step-console-content">${step.consoleLogs.map(l => `<div class="console-line">${this.escapeHtml(l)}</div>`).join('')}</div></div>`;
                }
                if (step.screenshot) {
                    html += `<div class="step-screenshot"><div class="step-screenshot-header">📷 Screenshot</div>
                        <a href="${step.screenshot}" target="_blank"><img src="${step.screenshot}" alt="Step Screenshot" class="step-screenshot-img"/></a></div>`;
                }
                if (step.error) {
                    html += `<div class="step-error"><div class="step-error-header">❌ Error</div>
                        <div class="step-error-message">${this.escapeHtml(step.error)}</div></div>`;
                    if (step.stackTrace) {
                        html += `<div class="step-stack-trace"><div class="step-stack-header">📜 Stack Trace</div>
                            <pre class="step-stack-content">${this.escapeHtml(step.stackTrace)}</pre></div>`;
                    }
                }
                html += `</div></div>`;
            }
        ...
        if (test.screenshots.length > 0) {
            html += `<div class="detail-section screenshots-section">
                <div class="section-header" onclick="toggleSection(this)"><span class="section-arrow">▼</span> Screenshots</div>
                <div class="section-content"><div class="screenshots-grid">
                    ${test.screenshots.map(s => `<div class="screenshot-item">
                        <a href="${s.path}" target="_blank" class="screenshot-link"><img src="${s.path}" alt="${s.name}" class="screenshot-preview"/></a>
        ...
        if (test.trace) {
            html += `<div class="detail-section traces-section">
                <div class="section-header" onclick="toggleSection(this)"><span class="section-arrow">▼</span> Traces</div>
                <div class="section-content"><a href="${test.trace}" download class="trace-download">📁 Download Trace</a></div></div>`;
        }

        if (test.video) {
            html += `<div class="detail-section videos-section">
                <div class="section-header" onclick="toggleSection(this)"><span class="section-arrow">▼</span> Videos</div>
                <div class="section-content">
                    <video controls class="test-video" src="${test.video}"></video>
                    <div class="video-link"><a href="${test.video}" target="_blank">📎 Open video</a></div>
                </div></div>`;
        }
    ...
    private getStyles(): string {
        ...
        :root {
            --primary:#059669;--primary-light:#10b981;--primary-dark:#047857;--primary-bg:#ecfdf5;
            --accent:#0d9488;--success:#22c55e;--danger:#ef4444;--warning:#f59e0b;--dark:#1e293b;
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        body{font-family:'Inter',-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;font-size:14px;line-height:1.6;background:linear-gradient(135deg,var(--gray-100) 0%,var(--primary-bg) 100%);min-height:100vh;color:var(--gray-700)}
        .header{background:linear-gradient(135deg,var(--primary) 0%,var(--accent) 50%,var(--primary-dark) 100%);color:white;padding:40px 20px;text-align:center;position:relative;overflow:hidden}
        .header h1{font-size:32px;font-weight:700;position:relative;text-shadow:0 2px 4px rgba(0,0,0,.2)}
        .header-subtitle{margin-top:8px;font-size:16px;opacity:.9;position:relative}
        ...
        .stat-card{background:white;border-radius:var(--radius);padding:24px;box-shadow:var(--shadow);border-left:4px solid var(--primary);transition:transform .2s,box-shadow .2s}
        .stat-card:hover{transform:translateY(-2px);box-shadow:var(--shadow-lg)}
        .stat-card.passed{border-left-color:var(--success)}.stat-card.failed{border-left-color:var(--danger)}.stat-card.skipped{border-left-color:var(--gray-400)}
        .stat-value{font-size:36px;font-weight:700;color:var(--dark);line-height:1}
        .stat-label{font-size:13px;color:var(--gray-500);text-transform:uppercase;letter-spacing:.5px;margin-top:8px;font-weight:500}
        ...
        .meta-label{font-size:12px;color:var(--gray-500);text-transform:uppercase;letter-spacing:.5px;font-weight:600}
        .meta-value{font-weight:600;color:var(--dark)}
        .env-badge,.browser-badge{display:inline-flex;align-items:center;gap:6px;padding:6px 14px;border-radius:20px;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:.5px}
        .env-badge{background:linear-gradient(135deg,var(--primary-light) 0%,var(--primary) 100%);color:white;box-shadow:0 2px 8px rgba(16,185,129,.3)}
        ...
        .filter-group strong{font-size:13px;color:var(--gray-600);text-transform:uppercase;letter-spacing:.5px}
        ...
        .status-badge.failed{background:linear-gradient(135deg,#ef4444 0%,#dc2626 100%);color:white;box-shadow:0 2px 8px rgba(239,68,68,.3)}
        .status-badge.skipped{background:var(--gray-200);color:var(--gray-600)}
        .screenshot-link,.video-link-cell,.trace-link-cell{display:inline-flex;align-items:center;gap:4px;padding:6px 12px;border-radius:var(--radius-sm);text-decoration:none;font-size:12px;font-weight:500;transition:all .2s}
        .screenshot-link{background:var(--primary-bg);color:var(--primary)}.screenshot-link:hover{background:var(--primary);color:white}
        .video-link-cell{background:#fef3c7;color:#d97706}.video-link-cell:hover{background:#f59e0b;color:white}
        .trace-link-cell{background:#ede9fe;color:#7c3aed}.trace-link-cell:hover{background:#8b5cf6;color:white}
        .tag{display:inline-block;padding:4px 10px;border-radius:12px;font-size:10px;font-weight:600;text-transform:uppercase;margin:2px;background:var(--primary-bg);color:var(--primary-dark);border:1px solid var(--primary-light)}
        .test-detail-row{background:var(--gray-50)!important}.test-detail-row td{padding:0!important}
        ...
        .detail-section{border-bottom:1px solid var(--gray-100)}.detail-section:last-child{border-bottom:none}
        .section-header{padding:16px 20px;background:var(--gray-50);cursor:pointer;font-weight:600;font-size:14px;display:flex;align-items:center;gap:10px;transition:background .2s}
        .section-header:hover{background:var(--gray-100)}
        .section-arrow{font-size:12px;color:var(--gray-400);transition:transform .3s}
        .section-collapsed .section-arrow{transform:rotate(-90deg)}.section-collapsed .section-content{display:none}
        ...
        .error-message{margin:0;color:var(--danger);font-family:monospace;font-size:13px;white-space:pre-wrap;word-break:break-word;line-height:1.6}
        .stack-details{margin-top:16px}.stack-details summary{cursor:pointer;color:var(--gray-500);font-size:13px;font-weight:500;padding:8px 0}
        .stack-trace-content{margin:12px 0 0;padding:16px;background:var(--dark);color:#a7f3d0;font-family:monospace;font-size:12px;border-radius:var(--radius-sm);overflow-x:auto;max-height:300px;overflow-y:auto;line-height:1.6}
        .steps-list{background:white;border-radius:var(--radius-sm);border:1px solid var(--gray-100);overflow:hidden}
        .step-item-container{border-bottom:1px solid var(--gray-100)}.step-item-container:last-child{border-bottom:none}
        .step-item{display:flex;align-items:center;padding:14px 16px;transition:background .2s}
        .step-item.expandable{cursor:pointer}.step-item.expandable:hover{background:var(--gray-50)}.step-item.failed{background:#fef2f2}
        .step-expand-icon{width:20px;font-size:10px;color:var(--gray-400);transition:transform .3s}
        ...
        .step-icon.passed{background:#dcfce7;color:var(--success)}.step-icon.failed{background:#fee2e2;color:var(--danger)}
        .step-name{flex:1;font-size:13px;font-weight:500;color:var(--dark)}
        .step-time{color:var(--gray-500);font-size:12px;font-family:monospace;background:var(--gray-100);padding:4px 10px;border-radius:12px}
        ...
        .step-console{margin-bottom:16px}.step-console-header{font-weight:600;color:var(--dark);margin-bottom:10px;font-size:13px}
        .step-console-content{background:var(--dark);color:#a7f3d0;padding:16px;border-radius:var(--radius-sm);font-family:monospace;font-size:12px;line-height:1.6;max-height:300px;overflow-y:auto}
        .console-line{padding:4px 0;border-bottom:1px solid rgba(255,255,255,.05)}.console-line:last-child{border-bottom:none}
        .step-screenshot{margin-bottom:16px}.step-screenshot-header{font-weight:600;color:var(--dark);margin-bottom:10px}
        ...
        .step-error{margin-bottom:16px}.step-error-header{font-weight:600;color:var(--danger);margin-bottom:10px}
        .step-error-message{background:#fef2f2;color:#b91c1c;padding:16px;border-radius:var(--radius-sm);font-family:monospace;font-size:12px;border-left:4px solid var(--danger)}
        .step-stack-trace{margin-top:12px}.step-stack-header{font-weight:600;color:var(--warning);margin-bottom:10px}
        .step-stack-content{background:#fffbeb;color:#92400e;padding:16px;border-radius:var(--radius-sm);font-family:monospace;font-size:11px;max-height:200px;overflow-y:auto;margin:0;border-left:4px solid var(--warning)}
        .screenshots-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:20px}
        .screenshot-item{background:white;border:1px solid var(--gray-200);border-radius:var(--radius);overflow:hidden;transition:transform .2s,box-shadow .2s}
        ...
        .trace-download{display:inline-flex;align-items:center;gap:8px;padding:12px 20px;background:linear-gradient(135deg,var(--primary-bg) 0%,#d1fae5 100%);color:var(--primary-dark);text-decoration:none;border-radius:var(--radius-sm);font-size:13px;font-weight:600;transition:all .2s;border:1px solid var(--primary-light)}
        ...
        .video-link{margin-top:12px;font-size:13px}.video-link a{color:var(--primary);font-weight:500}
        .modal{display:none;position:fixed;z-index:1000;left:0;top:0;width:100%;height:100%;background-color:rgba(0,0,0,.95);justify-content:center;align-items:center;backdrop-filter:blur(4px)}
        .modal.active{display:flex}
        .modal-content{max-width:95%;max-height:95%;object-fit:contain;border-radius:var(--radius);box-shadow:var(--shadow-xl)}
        .modal-close{position:absolute;top:20px;right:30px;color:white;font-size:40px;cursor:pointer;transition:color .2s,transform .2s}
        ...
        .report-footer p{font-size:14px;opacity:.9}
        .report-footer a{color:var(--primary-light);text-decoration:none;font-weight:600;transition:color .2s}
    ...
    private getScripts(): string {
        ...
        function toggleSection(header) { header.parentElement.classList.toggle('section-collapsed'); }
        function toggleStepDetails(stepElement, detailsId) {
            const details = document.getElementById(detailsId);
            if (details) {
                if (details.style.display === 'none') { details.style.display = 'block'; stepElement.classList.add('expanded'); }
                else { details.style.display = 'none'; stepElement.classList.remove('expanded'); }
            }
        }
        function filterByStatus(checkbox) {
            const allCb = document.querySelector('.status-filter[value="all"]');
            if (checkbox.value === 'all') { if (checkbox.checked) document.querySelectorAll('.status-filter:not([value="all"])').forEach(cb => cb.checked = false); }
            else { if (checkbox.checked && allCb) allCb.checked = false; if (!document.querySelectorAll('.status-filter:not([value="all"]):checked').length && allCb) allCb.checked = true; }
            applyFilters();
        }
        function filterByGroup(checkbox) {
            const allCb = document.querySelector('.group-filter[value="all"]');
            if (checkbox.value === 'all') { if (checkbox.checked) document.querySelectorAll('.group-filter:not([value="all"])').forEach(cb => cb.checked = false); }
            else { if (checkbox.checked && allCb) allCb.checked = false; if (!document.querySelectorAll('.group-filter:not([value="all"]):checked').length && allCb) allCb.checked = true; }
            applyFilters();
        }
        function applyFilters() {
            ...
            document.querySelectorAll('.test-row').forEach(row => {
                const rowStatus = row.classList.contains('passed') ? 'passed' : row.classList.contains('failed') ? 'failed' : 'skipped';
                const statusMatch = statusAll || statusFilters.includes(rowStatus);
                const tags = row.getAttribute('data-tags') || '';
                const groupMatch = groupAll || groupFilters.some(g => tags.toLowerCase().includes(g.toLowerCase()));
                const testId = row.getAttribute('data-test-id');
                const detailRow = document.getElementById('detail-row-' + testId);
                row.style.display = (statusMatch && groupMatch) ? '' : 'none';
                if (detailRow && detailRow.style.display !== 'none') detailRow.style.display = (statusMatch && groupMatch) ? 'table-row' : 'none';
            });
        }
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.screenshot-link').forEach(link => {
                link.addEventListener('click', function(e) {
                    ...
                    if (modal && modalImg) { modal.classList.add('active'); modalImg.src = this.href; }
                });
            });
            if (closeBtn) closeBtn.addEventListener('click', () => modal.classList.remove('active'));
            if (modal) modal.addEventListener('click', e => { if (e.target === modal) modal.classList.remove('active'); });
            document.addEventListener('keydown', e => { if (e.key === 'Escape' && modal) modal.classList.remove('active'); });
