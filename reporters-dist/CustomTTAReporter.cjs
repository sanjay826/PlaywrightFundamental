...
class CustomTTAReporter {
    testResults = [];
    fileGroups = new Map();
    suiteStats = { total: 0, passed: 0, failed: 0, skipped: 0, flaky: 0 };
    config;
    startTime = new Date();
    endTime = new Date();
    outputFile = 'tta-report/index.html';
    runId = '';
    testStepsMap = new Map();
    testStartTimeMap = new Map();
    testStepCounterMap = new Map();
    testCounter = 0;
    ...
    onTestEnd(test, result) {
        ...
        if (status === 'passed')
            fileGroup.stats.passed++;
        else if (status === 'failed' || status === 'timedOut')
            fileGroup.stats.failed++;
        else
            fileGroup.stats.skipped++;
    ...
    generateHistoryPage(reportDir) {
        const files = fs.readdirSync(reportDir).filter((f) => f.startsWith('report_') && f.endsWith('.html')).sort().reverse();
...
exports.default = CustomTTAReporter;
