...
Locator: locator('strong').filter({ hasText: 'Navigation commands' })
...
Call log:
  ...
  - waiting for locator('strong').filter({ hasText: 'Navigation commands' })
        ...
        - generic [ref=e219]:
          ...
          - generic [ref=e328]:
            ...
            - tablist [ref=e330]:
              ...
              - tab "WebElement Commands" [active] [ref=e335] [cursor=pointer]
            - generic [ref=e336]:
...
```ts
  ...
  55 |   }
  56 | 
  57 |   // Header Validation
  58 |   const navigationHeader = page
  59 |     .locator("strong")
  60 |     .filter({ hasText: "Navigation commands" });
  61 | 
  62 |   await expect(navigationHeader)
> 63 |     .toHaveText("Navigation Commands");
     |      ^ Error: expect(locator).toHaveText(expected) failed
  64 | 
  65 |   // Wait for Save Profile API Response
  66 |   const saveProfileResponsePromise = page.waitForResponse(
  67 |     response =>
  68 |       response.url().includes("/profile") &&
  69 |       response.request().method() === "POST"
  70 |   );
  71 | 
  72 |   // Save Profile
  73 |   await page
  74 |     .getByRole("button", { name: "Save profile" })
  75 |     .click();
  76 | 
  77 |   // Capture API Response
  78 |   const saveProfileResponse = await saveProfileResponsePromise;
  79 | 
  80 |   // Validate Response Status
  81 |   expect(saveProfileResponse.status()).toBe(200);
  82 | 
  83 |   // Convert Response to JSON
  84 |   const responseBody = await saveProfileResponse.json();
  85 | 
  86 |   // Assertions
  87 |   expect(responseBody.firstName).toBe("SKY");
  88 |   expect(responseBody.lastName).toBe("TSWS");
  89 |   expect(responseBody.gender).toBe("Male");
  90 |   expect(responseBody.yearsExperience).toBe("7");
  91 |   expect(responseBody.date).toBe("2026-05-05");
  92 |   expect(responseBody.profession).toBe("Automation Tester");
  93 |   expect(responseBody.tools).toBe("Selenium Webdriver");
  94 |   expect(responseBody.continents).toBe("Asia");
  95 | 
  96 |   console.log("Profile saved successfully and verified.");
  97 | 
  98 | });
```