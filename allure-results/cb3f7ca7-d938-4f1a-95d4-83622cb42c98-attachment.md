...
Locator:  locator('#selenium-tabs [role="tab"]').first()
Expected: "true"
Received: ""
Timeout:  5000ms
...
Call log:
  ...
  - waiting for locator('#selenium-tabs [role="tab"]').first()
    9 × locator resolved to <button role="tab" type="button" class="is-active" data-tab="browser" data-testid="tab-browser">Browser Commands</button>
      ...
      - region "Form practice workspace" [ref=e218]:
        - generic [ref=e219]:
          ...
          - generic [ref=e328]:
            ...
            - tablist [ref=e330]:
              - tab "Browser Commands" [active] [ref=e331] [cursor=pointer]
            ...
            - generic [ref=e336]:
              - strong [ref=e337]: Browser commands
              - text: — open and close the browser, get titles and URLs, manage windows.
              - code [ref=e338]: driver.getTitle(); driver.getCurrentUrl(); driver.close();
            - generic [ref=e339]:
              - generic [ref=e340]:
                - generic [ref=e341]: id
          ...
          - generic [ref=e346]:
            ...
            - generic [ref=e357]:
              ...
              - generic [ref=e360]:
                ...
                - text: =upload-image · download-file
          - generic [ref=e362]:
            - button "Save profile" [ref=e363] [cursor=pointer]
          ...
          - generic [ref=e366]:
            ...
            - generic [ref=e371]:
              ...
              - text: =button
          - generic [ref=e373]: Submitted profile JSON will appear here.
...
```ts
  ...
  53  | 
  54  |     await currentTab.click();
  55  | 
> 56  |     await expect(currentTab).toHaveAttribute(
      |                              ^ Error: expect(locator).toHaveAttribute(expected) failed
  57  |       "aria-selected",
  58  |       "true"
  59  |     );
  60  |   }
  61  | 
  62  |   // Header Validation
  63  |   const navigationHeader = page
  64  |     .locator("strong")
  65  |     .filter({ hasText: "Navigation commands" });
  66  | 
  67  |   await expect(navigationHeader)
  68  |     .toHaveText("Navigation commands");
  69  | 
  70  |   // Wait for Save Profile API Response
  71  |   const saveProfileResponsePromise = page.waitForResponse(
  72  |     response =>
  73  |       response.url().includes("/profile") &&
  74  |       response.request().method() === "POST"
  75  |   );
  76  | 
  77  |   // Save Profile
  78  |   await page
  79  |     .getByRole("button", { name: "Save profile" })
  80  |     .click();
  81  | 
  82  |   // Capture API Response
  83  |   const saveProfileResponse = await saveProfileResponsePromise;
  84  | 
  85  |   // Validate Response Status
  86  |   expect(saveProfileResponse.status()).toBe(200);
  87  | 
  88  |   // Convert Response to JSON
  89  |   const responseBody = await saveProfileResponse.json();
  90  | 
  91  |   // Assertions
  92  |   expect(responseBody.firstName).toBe("SKY");
  93  |   expect(responseBody.lastName).toBe("TSWS");
  94  |   expect(responseBody.gender).toBe("Male");
  95  |   expect(responseBody.yearsExperience).toBe("7");
  96  |   expect(responseBody.date).toBe("2026-05-05");
  97  |   expect(responseBody.profession).toBe("Automation Tester");
  98  |   expect(responseBody.tools).toBe("Selenium Webdriver");
  99  |   expect(responseBody.continents).toBe("Asia");
  100 | 
  101 |   console.log("Profile saved successfully and verified.");
  102 | 
  103 | });
```